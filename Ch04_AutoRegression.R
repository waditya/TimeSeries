...
# Now that you've modeled your data using the arima() command, you are ready to make simple forecasts based on your model. 
# The predict() function can be used to make forecasts from an estimated AR model. In the object generated by your predict() command, 
...
# This argument establishes the forecast horizon (h), or the number of periods being forceast. The forecasts are made recursively from 
# 1 to h-steps ahead from the end of the observed time series.

# In this exercise, you will make simple forecasts using an AR model applied to the Nile data, which records annual observations of the flow of the River Nile from 1871 to 1970.
